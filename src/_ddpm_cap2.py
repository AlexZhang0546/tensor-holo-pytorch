# -*- coding: utf-8 -*-
"""Single-sample network overfit: real DDPM (+UNet) joint training with SSIM loss.
Args: NF NL IDENT JOINT LR WSSIM DDPM_ONLY
DDPM_ONLY=1 -> freeze holonet (only ddpm trains); 0 -> joint train both.
"""
...
RES = 384
NF = int(sys.argv[1]) if len(sys.argv) > 1 else 32
NL = int(sys.argv[2]) if len(sys.argv) > 2 else 8
IDENT = int(sys.argv[3]) if len(sys.argv) > 3 else 2000
JOINT = int(sys.argv[4]) if len(sys.argv) > 4 else 3000
LR = float(sys.argv[5]) if len(sys.argv) > 5 else 1e-4
WSSIM = float(sys.argv[6]) if len(sys.argv) > 6 else 50.0
DDPM_ONLY = int(sys.argv[7]) if len(sys.argv) > 7 else 0
print("RES", RES, "NF", NF, "NL", NL, "IDENT", IDENT, "JOINT", JOINT,
      "LR", LR, "WSSIM", WSSIM, "DDPM_ONLY", DDPM_ONLY, flush=True)
...
ddpm = RealAmpPhaseDDPMNet(input_dim=3, output_dim=3, num_layers=NL,
...
torch.save({"ddpm_net_state_dict": ddpm.state_dict()}, os.path.join(BASE, "model/cap2_id_nf%d.pth" % NF))
...
torch.save({"ddpm_net_state_dict": ddpm.state_dict(),
           ...
           os.path.join(BASE, "model/cap2_joint_nf%d.pth" % NF))
