# -*- coding: utf-8 -*-
"""Field-ceiling test: directly optimize the altered field (free params)
with the stage2 joint loss on a single sample. If SSIM_img -> 0.9+, the
pipeline can represent the compensation and the network is the bottleneck.
Args: RES ITERS LR
"""
...
def build_field():
    amp = torch.sigmoid(amp_p) * 1.4142          # [0, sqrt(2)]
    phs = torch.sigmoid(phs_p)                   # [0, 1]
...
for it in range(ITERS):
    ...
    if it % 100 == 0 or it == ITERS - 1:
        ...
        print("[field] it %5d loss %.6f FS %.5f TV %.5f mean %.5f std %.5f | SSIM_img %.4f SSIM_amp %.4f (%.0fs)" % (
