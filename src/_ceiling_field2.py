# -*- coding: utf-8 -*-
"""Field-ceiling test v2: add SSIM loss option + VALID/SAME SSIM comparison.
...
WSSIM = float(sys.argv[4]) if len(sys.argv) > 4 else 50.0
...
def build_field():
    ...
    return compl_val(amp, (phs - 0.5) * 2.0 * np.pi), phs

def ssim_valid(img1, img2, data_range=1.0):
    # TF-style SSIM with VALID padding
    from src.utils.metrics import compute_ssim as cs
    k = 11; s = 1.5
    coords = torch.arange(k, dtype=torch.float32, device=img1.device) - (k - 1) / 2.0
    g = torch.exp(-(coords ** 2) / (2.0 * s ** 2)); g /= g.sum()
    w = (g[:, None] * g[None, :]).view(1, 1, k, k).repeat(img1.shape[1], 1, 1, 1).to(img1.dtype)
    C1 = (0.01 * data_range) ** 2; C2 = (0.03 * data_range) ** 2
    mu1 = F.conv2d(img1, w, padding=0, groups=img1.shape[1])
    mu2 = F.conv2d(img2, w, padding=0, groups=img1.shape[1])
    s11 = F.conv2d(img1 * img1, w, padding=0, groups=img1.shape[1]) - mu1 ** 2
    s22 = F.conv2d(img2 * img2, w, padding=0, groups=img1.shape[1]) - mu2 ** 2
    s12 = F.conv2d(img1 * img2, w, padding=0, groups=img1.shape[1]) - mu1 * mu2
...
for it in range(ITERS):
    opt.zero_grad()
    holo_altered, phs_for_reg = build_field()
    ...
    if it % 200 == 0 or it == ITERS - 1:
        with torch.no_grad():
            holo_altered_e, phs_e = build_field()
            st, mn = compute_ddpm_phase_loss(phs_e, pad=0, res_h=RES, res_w=RES)
            amp_c = amp_f[:, :, :RES, :RES]
            sa_same = compute_ssim(amp_c, amp_gt, data_range=1.0).item()
            sa_valid = ssim_valid(amp_c, amp_gt, 1.0).item()
        print("[field] it %5d loss %.6f FS %.5f TV %.5f mean %.5f std %.5f | SSIM_img %.4f SSIM_amp %.4f (valid %.4f) (%.0fs)" % (
            it, loss.item(), fs.item(), tv.item(), mn.item(), st.item(), ssim_img.item(), sa_same, sa_valid, time.time() - t0), flush=True)
