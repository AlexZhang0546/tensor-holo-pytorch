# -*- coding: utf-8 -*-
"""DDPM capacity test on single sample.
Args: NUM_FILTERS NUM_LAYERS IDENT JOINT LR
"""
...
IDENT = int(sys.argv[3]) if len(sys.argv) > 3 else 3000
JOINT = int(sys.argv[4]) if len(sys.argv) > 4 else 3000
LR = float(sys.argv[5]) if len(sys.argv) > 5 else 1e-4
print("RES", RES, "NF", NF, "NL", NL, "IDENT", IDENT, "JOINT", JOINT, "LR", LR, flush=True)
...
torch.save({"ddpm_net_state_dict": ddpm.state_dict()}, os.path.join(BASE, "model/ddpm_id_nf%d.pth" % NF))
...
torch.save({"ddpm_net_state_dict": ddpm.state_dict(),
           ...
           os.path.join(BASE, "model/ddpm_joint_nf%d.pth" % NF))
