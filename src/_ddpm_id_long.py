# -*- coding: utf-8 -*-
"""Long single-sample check: identity to convergence, then joint.
...
ident_iters = int(sys.argv[1]) if len(sys.argv) > 1 else 3000
joint_iters = int(sys.argv[2]) if len(sys.argv) > 2 else 500
...
print("[diag] amp_t min %.4f max %.4f mean %.4f | frac>sqrt2 %.4f frac>1.2 %.4f | phs_t std %.4f" % (
    amp_t.min().item(), amp_t.max().item(), amp_t.mean().item(),
    (amp_t > 1.4142).float().mean().item(), (amp_t > 1.2).float().mean().item(),
    phs_t.std().item()), flush=True)
...
for it in range(ident_iters):
    ...
    loss.backward()
    opt_id.step()
    if it % 200 == 0 or it == ident_iters - 1:
        with torch.no_grad():
            amp_a, phs_a = ddpm.forward_amp_phase(holo_shifted)
            l_a = F.l1_loss(amp_a, amp_t).item()
            l_p = F.l1_loss(phs_a, phs_t).item()
            sa = compute_ssim(amp_a, amp_t, data_range=1.0).item()
        print("[identity] it %5d loss %.6f (amp %.5f phs %.5f) SSIM_amp %.4f (%.0fs)" % (
            it, loss.item(), l_a, l_p, sa, time.time() - t0), flush=True)
torch.save({"ddpm_net_state_dict": ddpm.state_dict()}, os.path.join(BASE, "model/ddpm_id_long.pth"))
print("saved model/ddpm_id_long.pth", flush=True)
...
if joint_iters > 0:
    ...
    for it in range(joint_iters):
        ...
        loss.backward()
        opt_j.step()
        if it % 50 == 0 or it == joint_iters - 1:
            with torch.no_grad():
    ...
    torch.save({"ddpm_net_state_dict": ddpm.state_dict(),
               ...
               os.path.join(BASE, "model/ddpm_joint_long.pth"))
    print("saved model/ddpm_joint_long.pth", flush=True)
