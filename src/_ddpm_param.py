# -*- coding: utf-8 -*-
"""Parameterized single-sample stage2 check.
Args: RES IDENT_ITERS JOINT_ITERS LR
"""
...
IDENT = int(sys.argv[2]) if len(sys.argv) > 2 else 3000
JOINT = int(sys.argv[3]) if len(sys.argv) > 3 else 3000
LR = float(sys.argv[4]) if len(sys.argv) > 4 else 1e-4
print("RES", RES, "IDENT", IDENT, "JOINT", JOINT, "LR", LR, flush=True)
...
loader = create_dataloader(os.path.join(BASE, "data/validate_%d_v2/validate_04.tfrecord" % RES),
...
print("[diag] amp_t min %.4f max %.4f mean %.4f frac>sqrt2 %.4f | phs_t std %.4f" % (
    amp_t.min().item(), amp_t.max().item(), amp_t.mean().item(),
    (amp_t > 1.4142).float().mean().item(), phs_t.std().item()), flush=True)
...
torch.save({"ddpm_net_state_dict": ddpm.state_dict()}, os.path.join(BASE, "model/ddpm_id_%d.pth" % RES))
print("saved identity ckpt", flush=True)
...
torch.save({"ddpm_net_state_dict": ddpm.state_dict(),
           ...
           os.path.join(BASE, "model/ddpm_joint_%d.pth" % RES))
print("saved joint ckpt", flush=True)
