...
TF_DIR = "/root/autodl-tmp/ZhangRuixuan/tensor-holo"
...
def main():
    ...
    ddpm = {"active_max_ldi_layer": 0, "activate_ddpm": True,
    ...
    lp = {
        ...
        "weight_holo": 1.0, "weight_fs": 20.0,
        "weight_fs_tv": 20.0, "weight_std": 0.02, "weight_mean": 0.03,
    }
    ...
    train_ds = dict(ds_params(2), sample_count=3800, num_parallel_calls=4)
    test_ds = ds_params(2)
    val_ds = ds_params(2)

    model = TensorHolographyModel(
        hologram_params=hp, training_params=tp, ddpm_params=ddpm,
        model_params=mp, loss_params=lp, path_params=pp,
        train_dataset_params=train_ds, test_dataset_params=test_ds,
        validate_dataset_params=val_ds)

    (_, _, _, _, rgbd, holo_in, amp_in, phs_in, holo_out, amp_out, phs_out) = model._setup_train()
    print("rgbd", rgbd.shape)
    print("holo_in", holo_in.shape, "amp_in", amp_in.shape, "phs_in", phs_in.shape)
    print("holo_out", holo_out.shape, "amp_out", amp_out.shape, "phs_out", phs_out.shape)
    prop = optics.tf_propagator(
        (dataset_res + 2 * ddpm["padding"], dataset_res + 2 * ddpm["padding"]),
        pitch, hp["wavelengths"], method="as", double_pad=True)
    print("prop input_shape", prop.input_shape, "m_pad", prop.m_pad, "n_pad", prop.n_pad)
    print("unit_phase_shift shape", prop.unit_phase_shift.shape)
    print("fx", prop.fx.shape, "fy", prop.fy.shape)
    # print H shape explicitly
    H = optics.tf_compl_exp(12.0 * prop.unit_phase_shift, dtype=tf.complex64)
